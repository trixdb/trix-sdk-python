...
class GitHubResource(BaseSyncResource):
    ...
    def get_quality_profile(self, project_id: str) -> Dict[str, Any]:
        """Fetch the stored quality gate profile (preset + thresholds) for a project."""
        return self._client.get(f"/v1/projects/{project_id}/github/quality-profile")
    ...
    def suggest_reviewers(
        ...
        """Suggest reviewers ranked by commit-history expertise in the given files.

        file_paths: changed file paths to scope search (omit for project-wide)
        Returns: [{author, expertise_score, files_owned, ownership_pct, last_commit_at}]
        """
    ...
    def get_refactor_candidates(
        ...
        """Rank files by refactoring ROI score (CC + CogC + smells + suggestions + hotspot)."""
    ...
    def deep_pr_review(
        ...
        """One-call CQL-powered PR review using stored metrics (no live analysis).

        Returns: verdict, quality_score, review_body (markdown), inline_smells[], file_results[]
        """
        body: Dict[str, Any] = {
            "prNumber": pr_number,
        ...
        if repo_full_name:
            body["repoFullName"] = repo_full_name
        return self._client.post(f"/v1/projects/{project_id}/github/deep-pr-review", json=body)

    def orchestrate_pr_review(
        self,
        project_id: str,
        pr_number: int,
        repo_full_name: Optional[str] = None,
        ...
        Signals: pr_impact + function_smells + maintainability + test_coverage_gap.
        Returns: verdict, quality_score, review_body, inline_smells[], coverage_gaps[], reviewers[]
        """
        body: Dict[str, Any] = {
            "prNumber": pr_number,
            "limitFiles": limit_files,
    ...
    def generate_pr_description(
        ...
        Provide file_paths OR pr_number (to fetch changed files from GitHub).
        Returns: description (markdown), verdict, analyzed_files, metrics_summary, reviewers[]
        """
    ...
    def generate_ci_workflow(
        ...
        workflow_type: 'full' | 'pr_review' | 'scan' | 'quality_gate'
        gate: 'standard' | 'strict' | 'relaxed'
        Returns: workflows[]: [{filename, content, description}], instructions[]
        """
        return self._client.post(
    ...
    def get_test_coverage_gap(
        ...
        mode: 'files' | 'summary' | 'modules'
        Tiers: critical (≥50), high (≥20), medium (≥5), low (<5)
        """
        query: Dict[str, Any] = {
    ...
    def get_change_risk(
        ...
        """Pre-change blast radius and risk assessment.

        Answers "is it safe to modify these files right now?" using CC, MI, hotspot,
        debt, and test coverage gap signals. Returns overall_risk (LOW/MEDIUM/HIGH/CRITICAL),
        per-file breakdown, and recommended pre-change actions.
        """
        return self._client.post(
    ...
    def get_module_complexity(
        ...
        """Directory-level complexity view — SonarQube-style module quality aggregation.

        mode: 'summary' (group by dir prefix) | 'drill_down' (files in a module path)
        depth: directory grouping depth 1-5 (1='src', 2='src/auth')
        sort_by: module_score | avg_cc | avg_mi | total_hotspot | suggestion_count | file_count
        """
        query: Dict[str, Any] = {
    ...
    def get_toxic_files(
        ...
        """Multi-dimensional hall of shame — files failing on multiple quality dimensions.

        Each dimension scores 1 point: high_cc, low_mi, high_hotspot, high_debt, no_test, large_file.
        A file is toxic if toxicity_score >= min_score (default 3 out of 6).

        mode: 'files' (ranked list) | 'summary' (bucket counts + top failing dims + worst_5)
        min_score: minimum toxicity score 1-6 (default 3)
        """
        query: Dict[str, Any] = {
            "from": "toxic_files",
            "mode": mode,
    ...
    def evaluate_quality_gate(
        ...
        Evaluates 6 conditions against project-wide aggregates from code_file_metrics.
        preset: 'strict' | 'standard' (default) | 'relaxed'
        conditions: override specific thresholds e.g. {'avg_cc_lte': 12, 'avg_mi_gte': 50}

        Returns gate_status (PASSED|FAILED|NO_DATA), conditions_evaluated[], blocking_conditions[].
        """
    ...
    def get_refactor_priority(
        ...
        Scores are relative to the codebase max — tells you the highest-priority file to fix NOW.
        mode: 'files' (ranked) | 'summary'
        """
        query: Dict[str, Any] = {
    ...
    def batch_mark_findings(
        ...
        """Mark multiple SAST findings in one call.

        Each finding dict: {suggestion_id, status, fp_reason?, lifecycle_note?}
        Status values: false_positive | confirmed | in_progress | resolved | dismissed | open
