"""Goals resource for Trix SDK (ADR-035)."""
...
class GoalsResource(BaseSyncResource):
    ...
    Example:
        ...
        ... )
        >>> client.goals.update_progress(goal.id, progress=0.5)
    """
    ...
    def create(
        ...
        response = self._request("POST", "/goals", json=data.model_dump(exclude_none=True))
    ...
    def list(
        ...
        response = self._request("GET", "/goals", params=params)
        return GoalList.model_validate(response)

    def get(self, goal_id: str) -> Goal:
        """Get a goal by ID."""
        validate_id(goal_id, "goal")
        response = self._request("GET", f"/goals/{goal_id}")
        return Goal.model_validate(response)
    ...
    def transition_status(self, goal_id: str, status: str) -> Goal:
    ...
    def get_progress_history(
        ...
        validate_id(goal_id, "goal")
        params: Dict[str, Any] = {"limit": limit, "offset": offset}
        response = self._request("GET", f"/goals/{goal_id}/progress-history", params=params)
        return ProgressHistoryList.model_validate(response)

    def list_contributors(self, goal_id: str) -> List[GoalContributor]:
        """List contributors for a goal."""
        validate_id(goal_id, "goal")
        response = self._request("GET", f"/goals/{goal_id}/contributors")
    ...
    def remove_contributor(self, goal_id: str, persona_id: str) -> None:
        """Remove a contributor from a goal."""
        validate_id(goal_id, "goal")
        self._request("DELETE", f"/goals/{goal_id}/contributors/{persona_id}")
    ...
    def unlink_memory(self, goal_id: str, memory_id: str) -> None:
