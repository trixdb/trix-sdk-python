...
class Trix(SyncTransportMixin):
    ...
    def _get_headers(self) -> Dict[str, str]:
        ...
        ``httpx.Client`` defaults, and a baked-in ``application/json`` overrides
        the ``multipart/form-data; boundary=...`` httpx generates for file
        uploads. httpx sets the correct ``Content-Type`` per request from the
        ``json=`` / ``files=`` body, so leaving it unset keeps both paths right.
        """
        headers = {
