"""Main TrixDB client for sync and async operations."""
...
def _handle_response(response: httpx.Response) -> None:
    ...
    Raises:
        TrixDBError: On error responses
    """
...
class TrixDB:
    """
    Synchronous TrixDB client.

    Example:
        >>> # Using environment variable (recommended)
        >>> client = TrixDB.from_env()
        >>>
    ...
    def from_env(
        ...
        """
        Create a TrixDB client using credentials from environment variables.

        This is the recommended way to create a client for production use.

        Args:
            env_var: Environment variable name for API key (default: TRIXDB_API_KEY)
            base_url: Base URL (default: from TRIXDB_BASE_URL or https://api.trixdb.com)
        ...
        Example:
            >>> import os
            >>> os.environ["TRIXDB_API_KEY"] = "your_api_key"
            >>> client = TrixDB.from_env()
        """
        import os

        api_key = get_env_credential(env_var, required=True)
        base_url = base_url or os.environ.get("TRIXDB_BASE_URL", "https://api.trixdb.com")
    ...
    def __init__(
        ...
        """
        Initialize TrixDB client.

        For production use, prefer TrixDB.from_env() which reads credentials
        from environment variables.

        Args:
            api_key: API key for authentication
            jwt_token: JWT token for authentication (alternative to api_key)
            base_url: Base URL for TrixDB API (must be HTTPS)
            timeout: Request timeout in seconds
    ...
    def _get_headers(self) -> Dict[str, str]:
        ...
        headers = {
            ...
            "User-Agent": f"trixdb-python-sdk/{__version__}",
    ...
    def _request(
        ...
        """
        Make HTTP request to TrixDB API with retry logic.
...
class AsyncTrixDB:
    """
    Asynchronous TrixDB client.

    Example:
        >>> # Using environment variable (recommended)
        >>> async with AsyncTrixDB.from_env() as client:
    ...
    def from_env(
        ...
        """
        Create an async TrixDB client using credentials from environment variables.
    ...
    def __init__(
        ...
        """
        Initialize async TrixDB client.

        For production use, prefer AsyncTrixDB.from_env() which reads credentials
        from environment variables.
    ...
    async def _request(
        ...
        """
        Make async HTTP request to TrixDB API with retry logic.
    ...
    async def __aenter__(self) -> "AsyncTrixDB":
